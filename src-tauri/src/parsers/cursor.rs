...
use crate::parsers::{
    ...
    relocate_orphaned_tool_results, stable_user_anchor_id_from_parts, structurize_read_tool_output,
        ...
        if let Some(agent_turn) = wire::first_message(&turn_bytes, 1) {
            ...
            if let Some(user_ref) = wire::first_bytes(&agent_turn, 1) {
                ...
                if !user_blocks.is_empty() {
                    let anchor_id =
                        stable_user_anchor_id_from_parts(&hex_encode(turn_id), ts, &user_blocks);
            ...
            let shell_blocks = vec![ContentBlock::Text {
                text: format!("! {command}"),
            }];
            let shell_anchor =
                stable_user_anchor_id_from_parts(&hex_encode(turn_id), ts, &shell_blocks);
            turns.push(MessageTurn {
                id: String::new(),
                role: TurnRole::User,
                blocks: shell_blocks,
