...
use crate::parsers::{
    ...
    stable_user_anchor_id_from_parts, structurize_read_tool_output, title_from_user_text,
...
fn parse_updates(path: &Path) -> ParsedUpdates {
    ...
    let conversation_id = path.to_string_lossy();
    for (i, turn) in out.turns.iter_mut().enumerate() {
        turn.id = format!("grok-turn-{i}");
        if matches!(&turn.role, TurnRole::User) {
            // Grok timestamps have only second precision. Include the stable,
            // append-only turn position so repeated identical prompts submitted
            // within one second still receive distinct retry/history anchors.
            let anchor_source = format!("{conversation_id}:{}", turn.id);
            turn.anchor_id = Some(stable_user_anchor_id_from_parts(
                &anchor_source,
                turn.timestamp,
                &turn.blocks,
            ));
...
mod tests {
    ...
    #[test]
    fn identical_prompts_in_the_same_second_have_distinct_stable_anchors() {
        let updates = concat!(
            r#"{"method":"session/update","params":{"sessionId":"s","update":{"sessionUpdate":"user_message_chunk","content":{"type":"text","text":"continue"},"_meta":{"promptIndex":0}}},"timestamp":1783584019}"#,
            "\n",
            r#"{"method":"session/update","params":{"sessionId":"s","update":{"sessionUpdate":"agent_message_chunk","content":{"type":"text","text":"first"}}},"timestamp":1783584019}"#,
            "\n",
            r#"{"method":"session/update","params":{"sessionId":"s","update":{"sessionUpdate":"turn_completed","stop_reason":"end_turn"}},"timestamp":1783584019}"#,
            "\n",
            r#"{"method":"session/update","params":{"sessionId":"s","update":{"sessionUpdate":"user_message_chunk","content":{"type":"text","text":"continue"},"_meta":{"promptIndex":1}}},"timestamp":1783584019}"#,
            "\n",
            r#"{"method":"session/update","params":{"sessionId":"s","update":{"sessionUpdate":"agent_message_chunk","content":{"type":"text","text":"second"}}},"timestamp":1783584019}"#,
            "\n",
            r#"{"method":"session/update","params":{"sessionId":"s","update":{"sessionUpdate":"turn_completed","stop_reason":"end_turn"}},"timestamp":1783584019}"#,
            "\n",
        );
        ...
        let second = parser
            .get_conversation("019f45e3-e1ef-7690-a29f-fe2554382b49")
            .unwrap();
        let first_user_anchors: Vec<_> = first
            .turns
            .iter()
            .filter(|turn| matches!(turn.role, TurnRole::User))
            .map(|turn| turn.anchor_id.as_deref().expect("user anchor"))
            .collect();
        let second_user_anchors: Vec<_> = second
            .turns
            .iter()
            .filter(|turn| matches!(turn.role, TurnRole::User))
            .map(|turn| turn.anchor_id.as_deref().expect("user anchor"))
        ...
        assert_eq!(first_user_anchors.len(), 2);
        assert_ne!(first_user_anchors[0], first_user_anchors[1]);
        assert_eq!(first_user_anchors, second_user_anchors);
    }
