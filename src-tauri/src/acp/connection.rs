...
async fn apply_grok_preferred_model(
    ...
    let SessionConfigKindInfo::Select(sel) = &mut model_opt.kind else {
        return;
    };
...
async fn set_grok_config_option(
    ...
    let value_id = match value {
        SessionConfigCommandValue::ValueId(value_id) => value_id,
        SessionConfigCommandValue::Boolean(_) => {
            return Err(sacp::util::internal_error(
                "Grok model config requires a value_id",
            ));
        }
    };
...
fn resolve_claude_auto_compact_window_config(
    ...
    // Do not inject invalid values through _meta because claude-agent-acp
    // forwards options into the SDK as high-priority settings.
...
fn claude_raw_sdk_session_meta(
    ...
    if let Some(auto_compact_window) = auto_compact_config.value {
        // @agentclientprotocol/claude-agent-acp forwards _meta.claudeCode.options
        // directly into @anthropic-ai/claude-agent-sdk query({ options }).
        // Injecting autoCompactWindow here makes the native Claude Code runtime
        // enforce the configured window instead of falling back to its default
        // 200k context window for auto-compaction decisions.
    ...
    loop {
        ...
        match cmd {
                                ...
                                Some(ConnectionCommand::SetConfigOption {
                                    ...
                                    let set_result = if agent_type == AgentType::Grok {
                                        set_grok_config_option(
                                            &cx, &sid, state, emitter, config_id, value,
                                        )
                                        .await
                                    } else {
            ...
            Some(ConnectionCommand::SetConfigOption { config_id, value }) => {
                ...
                let sid = session.session_id().clone();
                let set_result = if agent_type == AgentType::Grok {
                    set_grok_config_option(&cx, &sid, state, emitter, config_id, value).await
                } else {
                    set_session_config_option(
                        &cx, &sid, state, emitter, agent_type, config_id, value,
                    )
...
mod tests {
    ...
    fn openclaw_session_requests_carry_no_mcp_servers() {
        ...
        let runtime_env = BTreeMap::new();
        let new_req =
            build_new_session_request(AgentType::OpenClaw, &cwd, Vec::new(), &runtime_env);
        assert!(
    ...
    fn synthesize_grok_config_options_yields_model_only_selector() {
        ...
        let SessionConfigKindInfo::Select(sel) = &opt.kind else {
