...
/// Connection-scoped gate for agent requests that would otherwise create an
/// interactive frontend surface. It stays closed through maintenance and any
/// delayed agent request until the next effective user prompt establishes a
/// public turn boundary.
#[derive(Default)]
struct MaintenanceInteractionGate {
    suppressed: AtomicBool,
}

impl MaintenanceInteractionGate {
    fn begin_maintenance(&self) {
        self.suppressed.store(true, Ordering::Release);
    }

    fn begin_user_prompt(&self) {
        self.suppressed.store(false, Ordering::Release);
    }

    fn suppresses_interactions(&self) -> bool {
...
fn current_model_id_from_opts(opts: &[SessionConfigOptionInfo]) -> Option<String> {
    opts.iter()
        .find_map(|option| match &option.kind {
            ...
            SessionConfigKindInfo::Select(_) | SessionConfigKindInfo::Boolean(_) => None,
        })
...
async fn run_connection(
    ...
    let maintenance_interaction_gate = Arc::new(MaintenanceInteractionGate::default());
        ...
        .on_receive_request(
            {
                ...
                let maintenance_interaction_gate = Arc::clone(&maintenance_interaction_gate);
...
async fn handle_grok_ask_user_question(
    ...
    state: &Arc<RwLock<SessionState>>,
    emitter: &EventEmitter,
    maintenance_interaction_gate: &MaintenanceInteractionGate,
    req: GrokAskUserQuestionRequest,
    responder: Responder<serde_json::Value>,
) {
    if maintenance_interaction_gate.suppresses_interactions() {
...
async fn apply_preferred_session_options(
    ...
    for (config_id, value) in preferred_config_values {
        ...
        let command = match resolve_preferred_config_command(option, value) {
            PreferredConfigResolution::Skip => continue,
            PreferredConfigResolution::Apply(command) => command,
            PreferredConfigResolution::Invalid => {
                tracing::warn!(
                    "[ACP] skipping invalid or unadvertised preferred config '{config_id}'"
                );
                continue;
            }
        };
        match set_session_config_option_inner(cx, &session_id, config_id.clone(), command).await {
            ...
            Err(e) => tracing::error!(
                "[ACP] failed to apply preferred config '{config_id}' on connect: {e}"
            ),
        }
    }
...
async fn handle_fork_or_exit(
    ...
    // Shared with request handlers across forks so private maintenance stays
    // unable to create a public interactive surface.
    maintenance_interaction_gate: &MaintenanceInteractionGate,
...
async fn run_conversation_loop<'a>(
    ...
    prompt_ledger: &background_watch::PromptLedger,
    // Shared with request handlers so maintenance-owned interactive requests
    // are rejected privately until a user prompt reopens the public boundary.
    maintenance_interaction_gate: &MaintenanceInteractionGate,
    ...
    loop {
        ...
        match cmd {
            Some(ConnectionCommand::Prompt {
                ...
                // provenance: its transcript turn must remain private.
                if is_maintenance {
                    prompt_ledger.record_maintenance_prompt_blocks(&blocks);
                } else {
                ...
                if prompt_blocks.is_empty() {
                    ...
                    // `send_prompt_inner`). Release a maintenance gate privately
                ...
                if is_maintenance {
                    maintenance_interaction_gate.begin_maintenance();
                } else {
                    maintenance_interaction_gate.begin_user_prompt();
                }
                                ...
                                SessionMessage::SessionMessage(dispatch) => {
                                    ...
                                    if let Err(e) = MatchDispatch::new(dispatch)
                                        .if_notification(
                                            async |notif: SessionNotification| {
                                                if is_maintenance {
                                                    // A private maintenance turn must not enter
                                                    // transcript storage, snapshots, or live UI.
                                                    return Ok(());
                                                }
                                    ...
                                    let set_result = if agent_type == AgentType::Grok {
                                        match value {
                                            ...
                                            SessionConfigCommandValue::Boolean(_) => {
                                                Err(sacp::Error::invalid_params().data(
                                                    "Grok config options require a select value",
                                                ))
                                            }
                                        }
                                    } else {
...
mod tests {
    ...
    #[test]
    fn maintenance_interaction_gate_stays_closed_until_user_prompt() {
        let gate = MaintenanceInteractionGate::default();
        assert!(
            !gate.suppresses_interactions(),
            "ordinary connection setup must allow interactive agent requests"
        );

        gate.begin_maintenance();
        assert!(
            gate.suppresses_interactions(),
            "maintenance-owned interactive requests must stay private"
        );

        gate.begin_user_prompt();
        assert!(
            !gate.suppresses_interactions(),
            "the next effective user prompt reopens interactive requests"
        );
    }
    ...
    fn config_command_value_must_match_the_advertised_kind_and_value() {
        ...
        assert!(!SessionConfigCommandValue::ValueId("true".to_string()).is_advertised_by(&boolean));
    }
    ...
    #[test]
    fn claude_window_merges_user_options_and_deduplicates_context_beta() {
        let dir = tempfile::tempdir().expect("create temporary Claude settings directory");
        ...
        std::fs::write(
            ...
            serde_json::json!({
                "_meta": {
                    "claudeCode": {
                        "options": {
                            "betas": ["existing-beta", CLAUDE_CONTEXT_1M_BETA],
                            "settings": {
        ...
        .expect("write temporary Claude settings");
        ...
        let meta = claude_raw_sdk_session_meta(AgentType::ClaudeCode, &runtime_env)
            .expect("Claude must have raw SDK meta");
        let json = serde_json::Value::Object(meta);
        let betas = json
            .pointer("/claudeCode/options/betas")
            .and_then(serde_json::Value::as_array)
            .expect("merged beta list");
        ...
        assert_eq!(
            betas
                ...
                .count(),
            1,
            "the 1M beta must appear exactly once"
        );
        assert_eq!(
            json.pointer("/claudeCode/options/settings/autoCompactWindow"),
            Some(&serde_json::json!(300000))
        );
        assert_eq!(
            json.pointer("/claudeCode/options/settings/preservedSetting"),
            Some(&serde_json::json!(true))
        );
        assert_eq!(
            json.pointer("/claudeCode/options/nested/preserved"),
    ...
    fn synthesize_grok_config_options_yields_model_and_effort_selectors() {
        ...
        let SessionConfigKindInfo::Select(model_sel) = &model.kind else {
            panic!("Grok model option must be a select")
        };
        ...
        let SessionConfigKindInfo::Select(effort_sel) = &effort.kind else {
            panic!("Grok effort option must be a select")
        };
