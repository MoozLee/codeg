...
use crate::acp::session_state::{ConnectionOrigin, SessionState};
...
/// Connection-scoped gate for agent requests that would otherwise create a
/// frontend interaction. A normal user prompt is the only boundary that reopens
/// it after a private maintenance turn.
#[derive(Default)]
...
pub async fn spawn_agent_connection(
    ...
    initial_state.connection_origin = connection_origin;
    ...
    let connection_thread = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            connection_rt.block_on(async move {
                ...
                // A user-requested disconnect of a private maintenance turn
                // must resolve only the maintenance caller. `run_connection`
                // clears its live gate before returning, so use the sticky flag
                // set by `ConnectionManager::disconnect` rather than origin.
                if !state_clone.read().await.suppress_public_terminal_events {
                    if let Err(e) = result {
...
async fn apply_grok_preferred_options(
    ...
    if let Some(pref) = preferred_config_values.get(GROK_MODEL_OPTION_ID).cloned() {
        ...
        let eligible = opts
            .iter()
            .find(|option| option.id == GROK_MODEL_OPTION_ID)
            .is_some_and(|option| match &option.kind {
                SessionConfigKindInfo::Select(select) => {
                    // Skip if already current, or the saved model is no longer offered.
                    select.current_value != pref
                        && select
                            .options
                            .iter()
                            .any(|candidate| candidate.value == pref)
                }
                SessionConfigKindInfo::Boolean(_) => false,
            });
        if eligible {
            match set_grok_model(cx, session_id, pref.clone(), None).await {
                Ok(()) => {
                    if let Some(option) = opts
                        ...
                        .find(|option| option.id == GROK_MODEL_OPTION_ID)
                    {
                        if let SessionConfigKindInfo::Select(select) = &mut option.kind {
                            select.current_value = pref.clone();
                        }
                    }
                    if !specs.is_empty() {
    ...
    if let Some(pref) = preferred_config_values.get(GROK_EFFORT_OPTION_ID) {
        ...
        if let Some(effort_opt) = opts.iter_mut().find(|o| o.id == GROK_EFFORT_OPTION_ID) {
            if let SessionConfigKindInfo::Select(select) = &mut effort_opt.kind {
                if &select.current_value != pref && select.options.iter().any(|o| &o.value == pref)
                {
                    if let Some(model_id) = model_id {
                        match set_grok_model(cx, session_id, model_id, Some(pref.clone())).await {
                            Ok(()) => select.current_value = pref.clone(),
                            Err(e) => tracing::error!(
    ...
    match set_grok_model(cx, session_id, model_id, effort).await {
        Ok(()) => {
            ...
            if let Some(mut opts) = current {
                if let Some(option) = opts.iter_mut().find(|option| option.id == config_id) {
                    if let SessionConfigKindInfo::Select(select) = &mut option.kind {
                        select.current_value = value_id.clone();
                    }
                }
                ...
                async move |req: RequestPermissionRequest,
                    ...
                    handle_permission_request(
                        ...
                        PermissionRequestScope {
                            agent_type,
                            maintenance_interaction_gate: &maintenance_interaction_gate,
                        },
                        req,
                        responder,
                    )
        ...
        .connect_with(agent, async move |cx| -> Result<(), sacp::Error> {
            ...
            let delegate_injection = if agent_supports_mcp && agent_delivers_wire_mcp(agent_type) {
                if let Some(inj) = delegation_injection.as_ref() {
                    // Task-engine launches carry the task_progress /
                    // task_complete tool group. Ownership is explicit rather
                    // than inferred from a UI window label.
                    let tasks_enabled = {
                        state.read().await.connection_origin == ConnectionOrigin::WorkTask
                    };
                    ...
                    match send_resume_session(&cx, resume_req).await {
                        Ok((resume_resp, grok_models_raw)) => {
                            ...
                            let loop_result = run_conversation_loop(
                                ...
                                supports_fork,
                                &prompt_ledger,
                                maintenance_interaction_gate.as_ref(),
...
struct PermissionRequestScope<'a> {
    agent_type: AgentType,
    maintenance_interaction_gate: &'a MaintenanceInteractionGate,
}
...
async fn handle_permission_request(
    ...
    perms: &PendingPermissions,
    cwd: &str,
    scope: PermissionRequestScope<'_>,
    req: RequestPermissionRequest,
    responder: Responder<RequestPermissionResponse>,
) {
    if scope.maintenance_interaction_gate.suppresses_interactions() {
        let _ = responder.respond(RequestPermissionResponse::new(
            RequestPermissionOutcome::Cancelled,
        ));
        return;
    ...
    if is_codex_plan_review(scope.agent_type, req.meta.as_ref()) {
        emit_with_state(
            state,
            emitter,
            AcpEvent::ToolCall {
...
fn route_prompt_response_completion(
    ...
    PromptResponseCompletionRoute::User
}

/// ACP notifications have no prompt correlation. After a maintenance response
/// ends, hold delayed notifications until the next normal user prompt boundary.
#[derive(Default)]
        ...
        match cmd {
            Some(ConnectionCommand::Prompt {
                ...
                if prompt_blocks.is_empty() {
                    ...
                    state.write().await.clear_turn_gate();
                ...
                loop {
                    tokio::select! {
                        update = session.read_update() => {
                            ...
                            match update {
                                ...
                                SessionMessage::StopReason(reason) => {
                                    if !is_maintenance && !tracked_terminal_tool_calls.is_empty() {
                                        poll_tracked_terminal_tool_calls(
                        ...
                        prompt_result = &mut prompt_response => {
                            ...
                            let raw_reason_str = stop_reason_to_str(reason);
                            match route_prompt_response_completion(
