...
mod tests {
    ...
    #[test]
    fn maintenance_prompt_is_private_and_not_surfaced() {
        let dir = tempfile::tempdir().unwrap();
        let path = temp_session(&dir);
        write_lines(&path, &[]);
        let ledger = PromptLedger::shared();
        ...
        ledger.record_maintenance_prompt_blocks(&[crate::acp::types::PromptInputBlock::Text {
        ...
        write_lines(
            ...
            &[
                ...
                &assistant_text("a1", "Compaction complete."),
            ],
        );
        assert!(
            tick_now(&mut ws, &ledger).is_none(),
            "private maintenance must not surface as an overlay"
        );
