...
impl PromptLedger {
    ...
    /// whole corresponding turn without publishing a background overlay.
    ...
    #[cfg(test)]
    fn record_private_text(&self, text: &str) {
        self.record_maintenance_prompt_blocks(&[crate::acp::types::PromptInputBlock::Text {
            text: text.to_string(),
        }]);
    }
}
...
enum Mode {
    ...
    Foreground,
    /// Records belong to private maintenance and must never enter the overlay.
    Private,
        ...
        if let Some(initiator_text) = turn_initiator_text(value) {
            match ledger.consume_matching(&initiator_text) {
                ...
                Some(LedgerEntryKind::Private) => {
                    // Claude persists app-managed maintenance in its own session
                    // file. Consume its complete turn without treating it as
...
mod tests {
    ...
    #[test]
    fn private_maintenance_prompt_is_not_surfaced_as_background_activity() {
        let dir = tempfile::tempdir().unwrap();
        let path = temp_session(&dir);
        write_lines(&path, &[]);
        let ledger = PromptLedger::shared();
        ...
        ledger.record_private_text("/compact");
        write_lines(
            &path,
            &[
                &user_prompt_array("u1", "/compact"),
                &assistant_text("a1", "Internal maintenance complete."),
            ],
        );

        assert!(
            tick_now(&mut ws, &ledger).is_none(),
            "maintenance transcript must not surface as an overlay"
        );
        assert!(matches!(&ws.mode, Mode::Private));
    }
