            ...
            if let (Some(cid), Some(fid)) = (link_conversation_id, link_folder_id) {
                ...
                if row.folder_id != fid {
                    return Err(AcpError::protocol(format!(
                        "fork_session folder mismatch for conversation {cid}: \
                         expected {}, got {fid}",
                        row.folder_id
                    )));
                }
...
mod tests {
    ...
    async fn send_prompt_linked_caller_id_is_noop_when_already_linked() {
        ...
        // ProcessExited, the rollback path fires Cancelled after the prompt fanout.
        ...
        let env_user_prompt = recv_first_acp_event(&mut rx).await;
        match env_user_prompt.payload {
            AcpEvent::UserPrompt {
                conversation_id,
                ...
                assert_eq!(conversation_id, Some(pre.id));
                assert_eq!(origin_connection_id, conn_id);
                assert_eq!(origin_client_id, None);
                assert!(!blocks.is_empty());
            }
            other => panic!("second event must be UserPrompt, got {other:?}"),
        }
        ...
        match env_cancelled.payload {
            ...
            other => panic!(
                "third event must be ConversationStatusChanged(Cancelled) after send failure, got {other:?}"
            ),
        }
    }
    ...
    async fn send_prompt_linked_writes_in_progress_and_emits_event() {
        ...
        // Cancelled-rollback path. We expect FOUR events in order:
        //   1. ConversationLinked
        //   2. ConversationStatusChanged(InProgress)  [pre-send write]
        //   3. UserPrompt                            [fanout to conversation clients]
        //   4. ConversationStatusChanged(Cancelled)   [rollback after send failure]
        let _ = mgr
            .send_prompt_linked(
                &db,
                conn_id,
        ...
        match env3.payload {
            ...
            other => panic!("third event must be UserPrompt, got {other:?}"),
        }
        ...
        match env4.payload {
            ...
            other => panic!(
                "fourth event must be ConversationStatusChanged(Cancelled) on send failure, got {other:?}"
            ),
        }
        // Ordering invariant: ConversationLinked < InProgress < UserPrompt < Cancelled.
        assert!(
            env2.seq > env1.seq && env3.seq > env2.seq && env4.seq > env3.seq,
            "event seqs must be strictly monotonic: linked={} in_progress={} user_prompt={} cancelled={}",
            env1.seq,
            env2.seq,
            env3.seq,
            env4.seq
        ...
        let env6 = recv_first_acp_event(&mut rx).await;
        match env6.payload {
            AcpEvent::UserPrompt {
                conversation_id,
                origin_connection_id,
            ...
            other => panic!("second send must emit UserPrompt before rollback, got {other:?}"),
        }
        let env7 = recv_first_acp_event(&mut rx).await;
        match env7.payload {
            AcpEvent::ConversationStatusChanged {
                conversation_id,
                status,
    ...
    async fn send_prompt_linked_rolls_back_to_cancelled_on_send_failure() {
        ...
        // Drain events: ConversationLinked → InProgress → UserPrompt → Cancelled, in order.
        ...
        match env_user_prompt.payload {
            ...
            other => panic!("expected UserPrompt before rollback, got {other:?}"),
        }
        let env_cancelled = recv_first_acp_event(&mut rx).await;
        match env_cancelled.payload {
            AcpEvent::ConversationStatusChanged {
        ...
        // Strict ordering: linked < in_progress < user_prompt < cancelled. The lifecycle
        ...
        assert!(
            env_in_progress.seq > env_linked.seq
                && env_user_prompt.seq > env_in_progress.seq
                && env_cancelled.seq > env_user_prompt.seq,
            "event seq must be strictly monotonic: linked={} in_progress={} user_prompt={} cancelled={}",
            env_linked.seq,
            env_in_progress.seq,
            env_user_prompt.seq,
            env_cancelled.seq,
    ...
    #[tokio::test]
    async fn fork_session_rejects_invalid_caller_link_without_mutating_state() {
        use crate::db::test_helpers;
        let db = test_helpers::fresh_in_memory_db().await;
        let folder_id = test_helpers::seed_folder(&db, "/tmp/fork-invalid-link").await;
        let mgr = ConnectionManager::new();
        {
            let mut map = mgr.connections.lock().await;
            map.insert(
                "c-invalid-link".into(),
                fake_connection("c-invalid-link", None),
            );
        }

        let err = mgr
            .fork_session(&db, "c-invalid-link", Some(99_999), Some(folder_id))
            .await
            .expect_err("missing caller row must be rejected before state link");
        assert!(
            err.to_string().contains("not found"),
            "error should mention missing row, got: {err}"
        );
        let state = mgr
            .get_state("c-invalid-link")
            .await
            .expect("connection still exists");
        assert_eq!(state.read().await.conversation_id, None);

        let other_folder_id = test_helpers::seed_folder(&db, "/tmp/fork-other-folder").await;
        let row = conversation_service::create(
            &db.conn,
            folder_id,
            AgentType::ClaudeCode,
            Some("Wrong Folder".into()),
            None,
        )
        .await
        .unwrap();
        let err = mgr
            .fork_session(&db, "c-invalid-link", Some(row.id), Some(other_folder_id))
            .await
            .expect_err("folder mismatch must be rejected before state link");
        assert!(
            err.to_string().contains("folder mismatch"),
            "error should mention folder mismatch, got: {err}"
        );
        let state = mgr
            .get_state("c-invalid-link")
            .await
