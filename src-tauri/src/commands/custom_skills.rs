...
mod tests {
    ...
    #[test]
    fn custom_skill_item_serializes_central_path_as_snake_case() {
        let value = serde_json::to_value(CustomSkillItem {
            id: "custom".into(),
            name: "Custom".into(),
            description: None,
            central_path: "/tmp/custom".into(),
        })
        .expect("serialize custom skill item");

        assert_eq!(value["central_path"], "/tmp/custom");
        assert!(value.get("centralPath").is_none());
    }
