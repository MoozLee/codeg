...
fn get_visible_conversation(
    ...
    // ACP transcripts have no durable prompt-origin marker. Do not infer an
    // internal maintenance turn from its text: legacy or external `/compact`
    // and `/summarize` messages are ordinary user history.
...
mod tests {
    ...
    #[test]
    fn preserves_historical_standalone_maintenance_commands() {
        struct StaticParser(ConversationDetail);

        impl AgentParser for StaticParser {
            ...
            fn get_conversation(
                ...
                if conversation_id == self.0.summary.id {
                    Ok(self.0.clone())
                } else {
                    Err(ParseError::ConversationNotFound(conversation_id.into()))
                }
            }
        }

        let parser = StaticParser(ConversationDetail {
            ...
            turns: vec![
                user_text_turn("turn-0", "/compact", at(-3)),
                assistant_text_turn("turn-1", "legacy response", at(-2), true),
                user_text_turn("turn-2", "/summarize", at(-1)),
            ],
            session_stats: None,
            transcript_watermark: None,
        });

        let detail = get_visible_conversation(&parser, "conversation-1").unwrap();
