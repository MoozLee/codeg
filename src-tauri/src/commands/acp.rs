...
/// fields the frontend may consume. Raw env/config inputs remain local to this
/// function and never enter the returned DTO.
pub(crate) fn build_context_runtime_config(
    agent_type: AgentType,
    runtime_env: &BTreeMap<String, String>,
    local_config_json: Option<&str>,
) -> ContextRuntimeConfigInfo {
    if matches!(agent_type, AgentType::Custom(_)) {
        return ContextRuntimeConfigInfo::default();
    }
    ...
    ContextRuntimeConfigInfo {
        ...
        native_auto_compact_window: (agent_type == AgentType::ClaudeCode)
            .then_some(configured_context_window_max_tokens)
            .flatten(),
    }
}
...
pub(crate) async fn acp_run_maintenance_command_core(
    ...
    operation_id: String,
    command: String,
) -> Result<crate::acp::types::MaintenanceCommandResult, AcpError> {
    manager
...
pub(crate) async fn acp_list_agents_core(db: &AppDatabase) -> Result<Vec<AcpAgentInfo>, AcpError> {
    ...
    for (idx, agent_type) in agent_types.into_iter().enumerate() {
        ...
        let uses_custom_skill_dir = agent_type == AgentType::Pi
            && setting
...
pub(crate) async fn acp_get_agent_config_core(
    agent_type: AgentType,
    db: &AppDatabase,
    ...
    let codex_model_catalog = (agent_type == AgentType::Codex)
        ...
        .flatten();
    let codex_sandbox_settings = (agent_type == AgentType::Codex)
        .then(|| parse_codex_sandbox_settings(codex_config_toml.as_deref().unwrap_or("")));
...
#[cfg(feature = "tauri-runtime")]
#[cfg_attr(feature = "tauri-runtime", tauri::command)]
pub async fn acp_get_agent_config(
    agent_type: AgentType,
    db: tauri::State<'_, AppDatabase>,
) -> Result<AcpAgentEditableConfig, AcpError> {
    acp_get_agent_config_core(agent_type, &db).await
}
...
mod tests {
    ...
    #[test]
    fn context_runtime_config_projects_only_safe_claude_fields() {
        let mut env = BTreeMap::new();
        env.insert("ANTHROPIC_MODEL".to_string(), "claude-opus-4-5".to_string());
        env.insert(
        ...
        env.insert("ANTHROPIC_API_KEY".to_string(), "secret-value".to_string());
        let config = r#"{
        ...
        let result = build_context_runtime_config(AgentType::ClaudeCode, &env, Some(config));

        assert_eq!(result.configured_model.as_deref(), Some("claude-opus-4-5"));
        assert_eq!(
            result.configured_model_source,
            Some(ConfiguredModelSource::AgentConfigEnv)
        );
        assert_eq!(result.configured_context_window_max_tokens, Some(200_000));
        assert_eq!(
            result.context_window_max_source,
            Some(ContextWindowMaxSource::AgentEnv)
        );
        assert_eq!(result.auto_compaction_enabled, Some(true));
        assert_eq!(result.auto_compaction_threshold, Some(80.0));
        assert_eq!(result.native_auto_compact_window, Some(200_000));
        let wire = serde_json::to_string(&result).unwrap();
        assert!(!wire.contains("secret-value"));
        assert!(!wire.contains("private/config.json"));
        assert!(!wire.contains("never-project"));
    }

    #[test]
    fn context_runtime_config_ignores_invalid_native_windows() {
        let mut env = BTreeMap::new();
        env.insert(
            "CLAUDE_CODE_AUTO_COMPACT_WINDOW".to_string(),
            "99999".to_string(),
        ...
        let result = build_context_runtime_config(AgentType::ClaudeCode, &env, None);

        assert_eq!(result.configured_context_window_max_tokens, None);
        assert_eq!(result.auto_compaction_threshold, None);
        assert_eq!(result.auto_compaction_enabled, None);
        assert_eq!(result.native_auto_compact_window, None);
    }
