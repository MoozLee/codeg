...
/**
 * Bridges backend conversation lifecycle events into the workspace's local
 * conversations list. The DB row is already updated by the backend before these
 * events fire, so status can patch in memory; a newly linked conversation may
 * not have a complete local summary yet, so it reconciles with a list refresh.
 *
  ...
  useAcpEvent((envelope: EventEnvelope) => {
    ...
    switch (envelope.type) {
      ...
      case "conversation_status_changed":
        store.updateConversationLocal(envelope.conversation_id, {
