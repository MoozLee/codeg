...
function connectionsReducer(
  ...
  switch (action.type) {
    ...
    case "HYDRATE_FROM_SNAPSHOT": {
      ...
      const mergedConfigStale = current.configStale || action.patch.configStale
      const mergedConfigStaleKind =
        current.configStaleKind ?? action.patch.configStaleKind
      const mergedBackgroundOutstanding = Math.max(
        current.backgroundOutstanding,
        action.patch.backgroundOutstanding
      )
      ...
      if (action.patch.eventSeq <= current.lastAppliedSeq) {
        if (
          ...
          mergedConfigStale === current.configStale &&
          mergedConfigStaleKind === current.configStaleKind &&
          mergedBackgroundOutstanding === current.backgroundOutstanding
        ) {
          return state
        }
        const next = new Map(state)
        next.set(action.contextKey, {
          ...
          configStale: mergedConfigStale,
          configStaleKind: mergedConfigStaleKind,
          backgroundOutstanding: mergedBackgroundOutstanding,
        })
        return next
      }
