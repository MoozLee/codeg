...
function connectionsReducer(
  ...
  switch (action.type) {
    ...
    case "SESSION_STARTED": {
      ...
      const next = new Map(state)
      const contextManagement = applyContextRuntimeIdentity(
    ...
    case "COMPACTION_STATUS_CHANGED": {
      ...
      const activeCompactionOperationId = terminal ? null : action.operationId
      const next = new Map(state)
      next.set(action.contextKey, {
        ...conn,
        contextManagement: {
...
export function AcpConnectionsProvider({ children }: { children: ReactNode }) {
  ...
  const setConfigOption = useCallback(
    async (contextKey: string, configId: string, value: string | boolean) => {
      const conn = storeRef.current.connections.get(contextKey)
      if (!conn || !configId.trim() || !isValidSessionConfigValue(value)) {
        return
      }
      const options =
        conn.configOptions ??
