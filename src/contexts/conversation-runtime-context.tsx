...
type Action =
  ...
  | {
      // Roll back an optimistic user turn that never reached the backend, or a
      // retry-edit turn identified by its shared anchor.
...
function reducer(
  ...
  switch (action.type) {
    ...
    case "REMOVE_OPTIMISTIC_TURN": {
      const current = state.byConversationId.get(action.conversationId)
      if (!current) return state
      const remainingOptimistic = current.optimisticTurns.filter((turn) => {
        if (action.id && turn.id === action.id) return false
        if (action.anchorId && turn.anchor_id === action.anchorId) return false
        return true
      })
      ...
      if (
        ...
        remainingLocal.length === current.localTurns.length
      ) {
        return state
      }
      return updateSessionInState(state, action.conversationId, (s) => ({
        ...s,
        optimisticTurns: remainingOptimistic,
        localTurns: remainingLocal,
        syncState: remainingOptimistic.length === 0 ? "idle" : s.syncState,
        activeTurnToken:
          remainingOptimistic.length === 0 ? null : s.activeTurnToken,
      }))
    }
...
export function ConversationRuntimeProvider({
  ...
  const refetchDetail = useCallback(
    (
      ...
      getFolderConversation(conversationId)
        .then((detail) => {
          if (!isLatestGeneration(targetConversationId, generation)) return
          dispatch({
            type: "FETCH_DETAIL_SUCCESS",
            conversationId: targetConversationId,
            detail,
  ...
  const removeOptimisticTurn = useCallback(
    (
      ...
      dispatch({
        type: "REMOVE_OPTIMISTIC_TURN",
        conversationId,
        ...(options?.match === "anchor"
          ? { anchorId: idOrAnchorId }
          : { id: idOrAnchorId }),
      })
    },
    []
