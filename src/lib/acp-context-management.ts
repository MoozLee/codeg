...
const EMPTY_CONTEXT_RUNTIME_CONFIG: ContextRuntimeConfigInfo = {
...
export function isMaintenanceCommandName(commandName: string): boolean {
  const normalized = commandName.trim().replace(/^\//, "").toLowerCase()
  return normalized === "compact" || normalized === "summarize"
}

/** Mirrors the backend's exact standalone maintenance-command reservation. */
export function isReservedMaintenancePromptBlocks(
  ...
  return (
    blocks.length === 1 &&
    block?.type === "text" &&
    /^\/(compact|summarize)$/i.test(block.text.trim())
  )
}
...
function hasNativeAutoCompactionContextConfig(
  agentType: AgentType,
  runtime: ContextRuntimeConfigInfo
): boolean {
  return (
    (agentType === "claude_code" || agentType === "grok") &&
...
export function deriveContextManagementFromAgentStatus(
  ...
  const runtime = agent.context_runtime_config ?? EMPTY_CONTEXT_RUNTIME_CONFIG
  const nativeAutoCompaction = hasNativeAutoCompactionContextConfig(
    agent.agent_type,
    runtime
  )
  ...
  return {
    ...
    compactionSupport: nativeAutoCompaction
      ? "native_managed"
...
export function findCompactionCommand(
  ...
  return (
    commands?.find((command) => isMaintenanceCommandName(command.name)) ?? null
  )
}
...
export function applyContextRuntimeIdentity(
  ...
  return {
    ...
    compactionSupport: nativeAutoCompaction ? "native_managed" : "unknown",
    compactionStatus: "idle",
    activeCompactionOperationId: null,
    lastCompactionError: null,
    runtimeConfig: {
...
/**
 * The usage update is the authoritative session window. An agent may clamp it
 * below static configuration, so scheduling and trigger-key re-arming must
 * calculate percentages against the same value.
 */
export function getCompactionContextWindowMaxTokens(
  usage: SessionUsageUpdateInfo | null,
  management: ContextManagementState
): number {
  return usage?.size && usage.size > 0
    ? usage.size
    : (management.runtimeContextWindowMaxTokens ??
        management.configuredContextWindowMaxTokens ??
        0)
}
...
export function getCompactionTriggerDecision(
  ...
  const contextMax = getCompactionContextWindowMaxTokens(usage, management)
  ...
  const normalizedCommandName = command.name
    .trim()
    .replace(/^\//, "")
    .toLowerCase()
  const commandName: MaintenanceCommand =
    normalizedCommandName === "compact" ? "/compact" : "/summarize"
  return {
    command: commandName,
