...
export function getCompactionTriggerDecision(
  ...
  // The current ACP usage event is the authoritative runtime window. Its size
  // can be smaller than static/configured metadata after an agent clamps a
  // session, so using configuredContextWindowMaxTokens here would understate
  // usage and suppress a needed maintenance turn.
  const contextMax =
    usage.size > 0
      ? usage.size
