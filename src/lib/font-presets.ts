...
export type FontSource = "system" | "bundled" | "discovered"
...
/** 界面字体可选项：无衬线 + 等宽全部允许（当前默认 JetBrains Mono 即等宽）。 */
...
function discoveredFont(font: SystemFontFamily): FontDef {
  const family = sanitizeFontFamily(font.family)
  const category: FontCategory = font.monospace ? "mono" : "sans"
  ...
  return {
    ...
    category,
    source: "discovered",
    stack: family ? `"${family}", ${fallback}` : fallback,
    ligatures: font.monospace,
  }
}

export function buildDiscoveredFonts(
...
export function systemFontFamilyFromId(id: string): string | null {
  ...
  return sanitizeFontFamily(id.slice("system:".length)) || null
}
...
export function resolveFontStack(
  ...
  const systemFontFamily = systemFontFamilyFromId(id)
  if (systemFontFamily) {
    return `"${systemFontFamily}", ${fallback}`
  }
...
export function fontSupportsLigatures(id: string): boolean {
  if (id === CUSTOM_FONT_ID || systemFontFamilyFromId(id)) return true
