...
describe("ACP context management", () => {
  ...
  it("identifies only reserved maintenance command names", () => {
    expect(isMaintenanceCommandName("compact")).toBe(true)
    expect(isMaintenanceCommandName(" /SUMMARIZE ")).toBe(true)
    expect(isMaintenanceCommandName("/compact this text")).toBe(false)
    expect(isMaintenanceCommandName("commit")).toBe(false)
  })

  it("reserves only exact standalone maintenance prompt blocks", () => {
    expect(
      isReservedMaintenancePromptBlocks([{ type: "text", text: " /COMPACT " }])
    ).toBe(true)
    expect(
      isReservedMaintenancePromptBlocks([
  ...
  it("never selects app-side triggering for native-managed Grok", () => {
    const runtimeState = deriveContextManagementFromAgentStatus(
      agentStatus(
        {
          auto_compaction_enabled: true,
          auto_compaction_threshold: 35,
  ...
  it("uses the live runtime window when a configured maximum is clamped", () => {
    const state = applyContextUsage(
      {
        ...DEFAULT_CONTEXT_MANAGEMENT,
        configuredContextWindowMaxTokens: 1_000_000,
      ...
      { used: 180_000, size: 200_000 }
    )
    const decision = getCompactionTriggerDecision({
      connectionId: "conn",
      sessionId: "session",
      status: "connected",
      usage: { used: 180_000, size: 200_000 },
      management: state,
      commands: [compactCommand],
    })

    expect(state.runtimeContextWindowMaxTokens).toBe(200_000)
    expect(state.runtimeContextWindowClamped).toBe(true)
    expect(decision).toEqual({
      command: "/compact",
      key: "conn:session:80:200000:90",
    })
  })

  it("normalizes an advertised compact command before dispatch", () => {
    const decision = getCompactionTriggerDecision({
      connectionId: "conn",
      sessionId: "session",
      status: "connected",
      usage: { used: 90, size: 100 },
      management: {
        ...DEFAULT_CONTEXT_MANAGEMENT,
        autoCompactionEnabled: true,
        compactionSupport: "agent_managed",
      },
      commands: [{ ...compactCommand, name: " /COMPACT " }],
    })

    expect(decision?.command).toBe("/compact")
  })
})
