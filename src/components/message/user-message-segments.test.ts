...
describe("parseUserMessageSegments", () => {
  ...
  describe("bare invocation tokens → skill badges", () => {
    ...
    it("does NOT badge a short symbol inside dollar math", () => {
      expect(parseUserMessageSegments("the value $x$ holds")).toEqual([
        { kind: "text", text: "the value $x$ holds" },
      ])
    })

    it("does NOT badge slash-like content inside dollar math", () => {
      expect(parseUserMessageSegments("solve $x + /y$ next")).toEqual([
        { kind: "text", text: "solve $x + /y$ next" },
      ])
    })

    it("still badges adjacent dollar-prefixed skill invocations", () => {
      const segments = parseUserMessageSegments("$deploy now $rollback")
      expect(
