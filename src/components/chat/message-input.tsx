...
import {
  ...
  restoreSerializedReferencesIntoEditor,
...
export function MessageInput({
  ...
  }, [effectiveDraftStorageKey, isExternalDraftEditing])
    ...
    const raf = requestAnimationFrame(() => {
      ...
      if (shouldHydrateExternalDraft) {
        ...
        if (
          ...
          if (editor) {
            restoreSerializedReferencesIntoEditor(editor, editingDraftText)
          } else {
  ...
  useEffect(() => {
    ...
    if (editingDraftBlocks && editingDraftBlocks.length > 0 && editor) {
      ...
      restoreSerializedReferencesIntoEditor(editor, retryDraftText)
    } else {
      editorRef.current?.setText(retryDraftText)
    }
    setComposerEmpty(editor ? isComposerEmpty(editor) : true)
  ...
  // Focus immediately when the active composer is ready, independently of ACP
  // connection state. The retry hydration above runs first and the next frame
  // places the caret after restored content.
  useEffect(() => {
  ...
  useEffect(() => {
    if (
      ...
      const raf = requestAnimationFrame(() => {
        ...
        if (editingDraftBlocks && editingDraftBlocks.length > 0 && editor) {
          hydrateFromBlocks(editor, editingDraftBlocks)
        } else if (editingDraftText != null && editor) {
          restoreSerializedReferencesIntoEditor(editor, editingDraftText)
        } else if (editingDraftText != null) {
  ...
  const handleSend = useCallback(() => {
    ...
    // Retry-edit mode must only resend when the current prompt is idle; unlike a
    // normal busy composer it must not enqueue a replacement as a new message.
    ...
    if (hasConfigOptions) {
      for (const option of availableConfigOptions) {
        if (option.kind.type === "boolean") {
          const currentValue = option.kind.current_value ? "true" : "false"
          result.push({
            ...
            onSelect: (value) =>
              onConfigOptionChange?.(option.id, value === "true"),
          })
          continue
        }
        ...
        const toOptions = (items: typeof kind.options) =>
          items
            .filter((item) => {
              ...
              : [
                  {
                    key: "__flat__",
                    name: null,
                    options: toOptions(kind.options),
                  },
                ]
        ).filter((group) => group.options.length > 0)
        const currentValueInvalid = (() => {
          const trimmed = kind.current_value.trim()
        ...
        const current = currentValueInvalid
          ? undefined
          : groups
        ...
        result.push({
          key: `config:${option.id}`,
          title: option.name,
          currentValue: currentValueInvalid ? "" : kind.current_value,
          currentLabel:
            current?.name ?? (currentValueInvalid ? "" : kind.current_value),
          groups,
