...
describe("MessageInput collapsed selectors popover", () => {
  ...
  it("selects a boolean config option from the cog Popover", async () => {
    const user = userEvent.setup()
    const onConfigOptionChange = vi.fn()
    const booleanOption: SessionConfigOptionInfo = {
      id: "auto-compact",
      name: "Auto compact",
      description: "Compact automatically",
      category: null,
      kind: {
        type: "boolean",
        current_value: false,
    ...
    const popover = await screen.findByRole("dialog", { name: settingsLabel })
    await user.click(within(popover).getByRole("button", { name: /On/ }))
    expect(onConfigOptionChange).toHaveBeenCalledWith("auto-compact", true)
  })
