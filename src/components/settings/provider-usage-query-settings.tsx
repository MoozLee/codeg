...
/**
 * Provider usage query management, rendered as an inline section inside
 * `/settings/model-providers`. Wraps the list, add/edit dialogs, and delete
 * confirm without its own ScrollArea / heading — the parent page owns the
 * outer scroll container and the "Provider Usage" heading.
 */
export function ProviderUsageQueryList() {
  ...
  function describeKinds(
    kinds: ProviderUsageConfigInfo["query_kinds"]
  ): string {
    if (!kinds || kinds.length === 0) return ""
    return kinds
      .map((k) =>
        (k as QueryKind) === "newapi_subscription"
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {summary.map(({ config, result }) => {
            ...
            const queryLabel = describeKinds(config.query_kinds)
            return (
              <div
                key={config.id}
