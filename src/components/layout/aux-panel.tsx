...
const TAB_ICONS: Record<AuxPanelTab, LucideIcon> = {
  ...
  user_messages: MessageSquareText,
  file_tree: Folder,
  changes: FolderPen,
  git_log: GitCommit,
}
...
const TAB_LABEL_KEY: Record<
  ...
  "userMessages" | "files" | "changes" | "commits"
> = {
...
const SEGMENTED_TABS_WIDTH = 160
...
export function resolveAuxTabView(
  ...
  return {
    showFolderTabs,
    effectiveTab:
      showFolderTabs || activeTab === "user_messages"
        ? activeTab
        : "session_details",
  }
}
...
export function AuxPanel() {
  ...
  const tabLabel = useCallback(
    (tab: AuxPanelTab) =>
      tab === "session_details" ? tDetails("menuLabel") : t(TAB_LABEL_KEY[tab]),
    [t, tDetails]
  )
  ...
  const renderTabTriggers = (compact: boolean) => {
    ...
    return TAB_ORDER.filter(
      (tab) =>
        tab === "session_details" || tab === "user_messages" || showFolderTabs
    ).map((tab) => {
    ...
    >
      ...
      >
        ...
        <TabsContent
          ...
          className="mt-0 flex-1 min-h-0 overflow-hidden"
        >
          {mountedTabs.has("user_messages") ? <UserMessagesTab /> : null}
        </TabsContent>
        <TabsContent
          value="file_tree"
          forceMount
