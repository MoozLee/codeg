...
describe("resolveAuxTabView", () => {
  ...
  it("collapses folder tabs to Session Details when no folder is open", () => {
  ...
  it("keeps User Messages available without a folder workspace", () => {
    expect(resolveAuxTabView("user_messages", null, true)).toEqual({
      showFolderTabs: false,
      effectiveTab: "user_messages",
    })
  })
...
describe("shouldCollapseAuxTabs", () => {
  ...
  it("collapses at the default width when the win/linux caption is reserved", () => {
    // 320 − 12 − 254 = 54 available < 172: the wider reservation forces a
