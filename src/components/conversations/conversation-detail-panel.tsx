...
function hasImmediatePromptingSignal(
  hasImmediateLiveMessageSignal: boolean,
  pendingPermission: unknown,
  pendingQuestion: unknown,
  claudeApiRetry: unknown
): boolean {
  return Boolean(
    hasImmediateLiveMessageSignal ||
    pendingPermission ||
    pendingQuestion ||
    claudeApiRetry
  )
...
const ConversationTabView = memo(function ConversationTabView({
  ...
  const tabPersistenceMode = useTabStore((s) => s.tabPersistenceMode)
  ...
  const hasPromptingLiveMessage = useConversationRuntimeStore(
    (s) => s.byConversationId.get(effectiveConversationId)?.liveMessage != null
  )
  const hasImmediateLiveMessageSignal = useConversationRuntimeStore((s) => {
    const liveMessage = s.byConversationId.get(
      effectiveConversationId
    )?.liveMessage
    if (!liveMessage) return false
    if (liveMessage.content.length === 0) return true
  ...
  useEffect(() => {
    if (connStatus === "prompting" && hasImmediateLiveMessageSignal) {
      lastPromptingHadLiveMessageRef.current = hasPromptingLiveMessage
  ...
  }, [connStatus, hasImmediateLiveMessageSignal, hasPromptingLiveMessage])
  ...
  useEffect(() => {
    const effectiveHasLiveMessage =
      hasPromptingLiveMessage ||
      (connStatus === "prompting"
        ? lastPromptingHadLiveMessageRef.current
        : false)
...
export function ConversationDetailPanel({
  ...
  const activeTabFolder = useMemo(
    () =>
      activeTab
        ? (allFolders.find((entry) => entry.id === activeTab.folderId) ?? null)
        : null,
