...
let persistenceMode: TabPersistenceMode = "shared"
let workspaceBootstrapState: WorkspaceBootstrapState | undefined
...
function buildBootstrapTab(
  bootstrapState: WorkspaceBootstrapState
): TabItemInternal | null {
  const target = bootstrapState.target
  ...
  const folderId = target.folderId ?? 0
  const resolvedAgent =
    target.agentType ?? resolveAgentForFolder(folderId, null).agentType
  return {
    ...
    isPinned: true,
    workingDir: target.workingDir ?? undefined,
  }
}
...
export const useTabStore = create<TabStoreState>()((set, get) => ({
  ...
  setTabFolder: (tabId, folderId, workingDir) => {
    const prev = get().rawTabs
    const target = prev.find((tab) => tab.id === tabId)
    if (!target) return
    if (target.folderId === folderId && target.workingDir === workingDir) return
    set({
      rawTabs: prev.map((tab) =>
    ...
    void (async () => {
      try {
        if (persistenceMode === "window-local") {
          ...
          if (raw) {
            try {
              restored = restoreWindowLocalTabs(
                JSON.parse(raw) as WindowLocalOpenedTab[]
              )
            } catch {
              restored = []
            }
          ...
          if (restored.length === 0 && workspaceBootstrapState?.target) {
            if (typeof window !== "undefined") {
              consumeWorkspaceBootstrap(window.location.search)
            }
            const initialTab = buildBootstrapTab(workspaceBootstrapState)
            if (initialTab) restored = [initialTab]
          }

          const activeFromStorage = raw
            ? (() => {
                try {
                  const parsed = JSON.parse(raw) as WindowLocalOpenedTab[]
                  const active = parsed.find((item) => item.is_active)
                  return active
                    ? (restored.find((tab) => tab.id === active.id)?.id ?? null)
                    : null
                } catch {
                  return null
                }
            ...
            : null
          const restoredActive = activeFromStorage ?? restored[0]?.id ?? null
          set({ rawTabs: restored, activeTabId: restoredActive })
          recomputeTabs()
          lastSavedPayload = JSON.stringify(
            serializeWindowLocalTabs(restored, restoredActive)
          )
          return
        }
        ...
        if (!cancelled) {
          ...
          if (persistenceMode === "window-local") return
  ...
  runSaveEffect: () => {
    ...
    if (persistenceMode === "window-local") {
      const items = serializeWindowLocalTabs(st.rawTabs, st.activeTabId)
  ...
  syncActiveFolderId: () => {
    const st = get()
    if (previousActiveTabId !== st.activeTabId) {
      previousActiveTabId = st.activeTabId
      if (st.activeTabId != null) {
        set({ activeTabActivationSeq: st.activeTabActivationSeq + 1 })
      }
    }
  ...
  configurePersistence: (mode, bootstrapState) => {
    persistenceMode = mode
    workspaceBootstrapState = bootstrapState
    const isTileMode =
      typeof window !== "undefined" &&
      readScopedStorageItem(persistenceMode, TILE_MODE_STORAGE_KEY) === "true"
    set({ tabPersistenceMode: mode, isTileMode })
  },
