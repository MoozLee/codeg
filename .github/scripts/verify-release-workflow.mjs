...
const signatureVerifier = read("src-tauri/src/update/verify.rs")
...
assert(
  ...
  "server signature verification must use the configured updater public key"
)
assert(
...
const upstreamWorkTaskMigrations = [
  "m20260801_000001_work_task",
  "m20260801_000002_work_task_p2",
  "m20260801_000003_work_task_template",
]
const workTaskStart = migrationRegistrations.indexOf(
  upstreamWorkTaskMigrations[0]
)
assert(
  workTaskStart > customAgentStart &&
    migrationRegistrations
      .slice(workTaskStart, workTaskStart + upstreamWorkTaskMigrations.length)
      .every((name, index) => name === upstreamWorkTaskMigrations[index]),
  "upstream work-task migrations no longer follow custom-agent migrations"
)
