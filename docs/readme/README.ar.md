...
هذا المستودع هو تفرّع من [xintaofei/codeg](https://github.com/xintaofei/codeg). يبقى فرع `main` قريبًا من المشروع الأصلي، بينما قد يحمل فرع `release` في هذا التفرّع ميزات موجّهة للمستخدم قبل الأصل أو بما يتجاوزه. من الأمثلة الحديثة: التنقّل في سجل رسائل المستخدم، وإعدادات قابلة للتخصيص لهدف فتح الملفات والخطوط عبر البيئات المختلفة. وتجد أدناه أيضًا مزايا أخرى خاصة بهذا التفرّع مثل **قنوات الدردشة**، ونشر **`codeg-server`**، و**دعم Docker**، و**مُنشئ المشروع**.
...
1. أنشئ قناة في **الإعدادات → قنوات الدردشة** (اختر Telegram أو Lark أو iLink)
2. أدخل رمز البوت (Telegram)، أو بيانات اعتماد التطبيق (Lark)، أو امسح رمز QR لتسجيل الدخول (iLink) — مخزّنة بأمان في حلقة مفاتيح نظام التشغيل
...
- [LinuxDO](https://linux.do) — المجتمع الذي بدأ منه كل شيء
- [ACP](https://agentclientprotocol.com) — بروتوكول عميل الوكيل (ACP) هو الأساس الذي يمكّن Codeg من الاتصال بوكلاء متعددين
